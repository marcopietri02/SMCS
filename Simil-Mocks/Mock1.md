...
2. Calcolare la probabilità condizionata che un modulo contenga un bug dato che è scritto in Python (P(B | L_2)).
3. Utilizzando la **Formula di Bayes**, calcolare la probabilità che un modulo sia scritto in Java, sapendo che è stato rilevato un bug (P(L_1 | B)).
...
- **Sezione I:**
    - **Media campionaria:** \bar{Y} = 26.0000 ms. Calcolo: \frac{(15 \cdot 4) + (25 \cdot 3) + (35 \cdot 1) + (45 \cdot 2)}{10} = \frac{260}{10}.
    - **Mediana:** \approx 23.3333 ms. (Classe mediana [20, 30)).
    - **Deviazione standard:** s \approx 11.9722 ms. Calcolo: \sqrt{\frac{\sum f_i(y_i - \bar{Y})^2}{N-1}} = \sqrt{\frac{1290}{9}}.
- **Sezione II:**
...
- **Sezione III:**
    ...
    - P(B | L_2) = \frac{0.05}{0.40} = 0.1250.
    - **Bayes:** P(L_1 | B) = \frac{P(B | L_1)P(L_1)}{P(B)} = \frac{0.08}{0.20} = 0.4000.
- **Sezione IV:**
    - **Binomiale:** P(X=2) = \binom{8}{2} (0.15)^2 (0.85)^6 \approx 0.2376.
    - **Poisson:** P(X=4) = \frac{e^{-3} \cdot 3^4}{4!} \approx 0.1680.
- **Sezione V:**
    - P(X > 65) = P(Z > \frac{65-50}{10}) = P(Z > 1.5) \approx 0.0668.
    - **90° Percentile:** k \approx 62.8200 ms (utilizzando z_{0.90} \approx 1.282).
    - **Nota teorica:** La standardizzazione permette di confrontare diverse distribuzioni normali e utilizzare tavole statistiche universali poiché ogni X \sim N(\mu, \sigma^2) può essere mappata su Z \sim N(0, 1).
